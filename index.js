...
contract Oracles =
  ...
  stateful entrypoint init() : state =  //Función de Inicio
    ...
    { greeter_oracle = greeter_oracle, id_query = {}  }
...
const contractAddress = 'ct_WwBKX3ZqjBiB5Ggfej3F43sJKAKcKKVob5fz8fMzXWgHfa9QV';
...
//If someone clicks to respond oracle,  execute respond
//Si alguien hace clic para incluir respuesta oráculo, ejecute respond
$('#respondBtn').click(async function(){
	$("#loader").show();
	const adrress = ($('#adrress').val());
	const idquery = ($('#idquery').val());
	const string = ($('#string').val());
	client = await Ae.Aepp();
	await contractCall('respond', [adrress,idquery,string], 0);
	$("#loader").hide();
});
...
$('#getQuestionBtn').click(async function(){
	...
	const idquery = ($('#idquery').val());
	client = await Ae.Aepp();
	const consul = await callStatic('getQuestion',[adrress,idquery]);
	document.getElementById("textnotice").innerHTML = "Question: "+consul;
	div = document.getElementById('notice');
	div.style.display = '';
	$("#loader").hide();
});
